...
0
|| self::check('default\category.tpl', 'D:\Web\Site\tblog\trunk\admin\application\View\default\category.tpl', 1404152141)
;?>
...
<div id="main">
  <div class="main_box">
    <h2>添加/修改分类<?php if($action == 'index') { ?>｜<a class="cursor" onclick="open_cat()">全部展开</a>｜<a class="cursor" onclick="close_cat()">全部收缩</a><?php } ?></h2>    
<?php if($action == 'add' ) { ?>    
    <form method="post" action="<?php echo PHP_SELF; ?>?m=category.save" onSubmit="return checkForm(this);">
    <input type="hidden" value="<?php echo $editinfo->cat_id;?>" name="cat_id" id="cat_id">
    ...
    <tbody>
		...
		<tr>
			...
			<td class="td_left"> <input type="text" size="35" name="category_nicename" id="category_nicename" value="<?php echo $editinfo->category_nicename;?>"></td>
		...
		<tr>
			...
			<td class="td_left"> <input type="text" size="35" name="category_keywords" id="category_keywords" value="<?php echo $editinfo->category_keywords;?>"></td>
            <td class="td_left hui">keywords</td>
        </tr>

		<tr>
        	<td class="td_right">描述</td>
			<td class="td_left" colspan="2"><textarea id="category_description" rows="4" style="height:50px" cols="70" name="category_description"><?php echo $editinfo->category_description;?></textarea> description</td>
        </tr>  
        
		<tr>
        	<td class="td_right">内容</td>
			<td class="td_left" colspan="2"><textarea name="category_content" id="category_content" rows="60" cols="100" class="xheditor editor" style="height:270px"><?php echo $editinfo->category_content;?></textarea><br />&nbsp;&nbsp;通常用于企业简介之类的单页面用途</td>			
        </tr>          
        
        
		<tr>
        	<td class="td_right">分类排序</td>
			<td class="td_left"><input type="text" size="5" name="cat_order" id="cat_order" value="<?php echo $editinfo->cat_order;?>"></td>
            <td class="td_left hui">数字越大的排第一</td>
        </tr>
        
		<tr>
        	<td class="td_right">前台链接文件</td>
			<td class="td_left"><input type="text" size="10" name="urlfile" id="urlfile" value="<?php echo $editinfo->urlfile;?>"></td>
...
//以后jquery中的都用jq代替即可。 
jq('#category_content').xheditor({upImgUrl:"<?php echo PHP_SELF; ?>?m=tool.uploadImg",upImgExt:"jpg,jpeg,gif,png"});
</script>      
<?php } ?>
