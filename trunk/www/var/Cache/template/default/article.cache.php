...
0
|| self::check('default\article.tpl', 'D:\Web\Site\tblog\trunk\admin\application\View\default\article.tpl', 1404649259)
;?>
...
<?php if($action == 'add' ) { ?>    
    <h2>添加文章</h2>
<form name="forms" id="forms" action="<?php echo PHP_SELF; ?>?m=article.save" method="post"  onSubmit="return chkForm();">
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="t_list">
      <tbody>
      <tr>
        <td class="td_right_f00" width="150">标题：</td>
        <td class="td_left" colspan="2"><input type="text" size="100" value="<?php echo $editinfo->title;?>" id="title" name="title">
	  </td>
      </tr>
       <tr>
      ...
      <tr>
          ...
          <td class="td_left"> <input type="text" size="35" name="keywords" id="keywords" value="<?php echo $editinfo->keywords;?>"> keywords</td>
      </tr>

      <tr>
          <td class="td_right">描述</td>
          <td class="td_left" colspan="2"><textarea id="description" rows="4" style="height:50px" cols="70" name="description"><?php echo $editinfo->description;?></textarea> description</td>
      </tr>        
            
      <tr>
        <td class="td_right">内容：</td>
        <td class="td_left">
		<textarea name="content" id="content" rows="60" cols="100" class="editor"><?php echo $editinfo->content;?></textarea>        
		</td>
      </tr>     
       ...
       <tr>
        ...
        <?php echo $comment_status_array_option;?>
        </select>
		</td>
      </tr>                 
      ...
      <tr>
        ...
        <td class="td_left"><input type="text" value="<?php echo $editinfo->article_order;?>" id="article_order" name="article_order"></td>
      </tr>
      
      <tr>
        <td class="td_right">别名：</td>
        <td class="td_left"><input type="text" value="<?php echo $editinfo->name;?>" id="name" name="name">
      ...
      <tr>
        ...
        <?php if(is_array($tag_info_array)) foreach($tag_info_array AS $k => $v) { ?>
        <span><a class="ntdelbutton" id="post_tag-check-num-<?php echo $v->tag_id;?>">X</a><p><?php echo $v->tag_name;?></p></span>
        <?php } ?>
        </div>
        ...
        <?php if(is_array($tag_info_array)) foreach($tag_info_array AS $k => $v) { ?>
		<input id="tag_id_<?php echo $v->tag_id;?>" value="<?php echo $v->tag_id;?>" name="tag_id[]"><input id="tag_name_<?php echo $v->tag_name;?>" value="<?php echo $v->tag_id;?>" name="tag_name[]">        
        <?php } ?>
        </p>     
        </td>
      </tr>            
      ...
      <tr>
		...
		&nbsp;&nbsp;<input type="button" value="上传" onclick="return ajaxFileUpload('thumb_upload','<?php echo PHP_SELF; ?>?m=tool.uploadImageByAjax&filename=thumb_upload&action=article','#thumb_loading', 'thumb', 'thumb_preview', 'thumb_image_id');" id="thumbupload" name="thumbupload">    
		<img style="display:none;" src="<?php echo $BASE;?>js/loading.gif" id="thumb_loading">
		<input type="hidden" name="thumb_image_id" id="thumb_image_id" value="<?php echo $editinfo->thumb;?>"/>		
		</td>
      </tr>     
      ...
      <tr>
        ...
        <td class="td_left">
          <input type="hidden" name="aid" value="<?php echo $editinfo->article_id;?>" />
          <input type="hidden" name="author" id="author" value="<?php echo $editinfo->author;?>" />             
...
<script src="<?php echo STATIC_URL; ?>js/xheditor/xheditor-1.1.14-zh-cn.min.js" type="text/javascript"></script>
...
function pageInit()
{
	...
	editor=jq('#content').xheditor({plugins:allPlugin,tools:'full',upLinkUrl:"<?php echo PHP_SELF; ?>?m=tool.uploadImg",upLinkExt:"zip,rar,txt",upImgUrl:"<?php echo PHP_SELF; ?>?m=tool.uploadImg",upImgExt:"jpg,jpeg,gif,png",upFlashUrl:"<?php echo PHP_SELF; ?>?m=tool.uploadImg",upFlashExt:"swf",upMediaUrl:"<?php echo PHP_SELF; ?>?m=tool.uploadImg",upMediaExt:"avi",loadCSS:'<style>pre{margin-left:2em;border-left:3px solid #CCC;padding:0 1em;}</style>',}); 
}
...
function changeAuthor(uid)
{
	...
	<?php if(is_array($user_array)) foreach($user_array AS $k => $v) { ?>
	user_array[<?php echo $v->uid;?>] = '<?php echo $v->nicename;?>';<?php } ?>
...
jq(document).ready(function(){
	...
	jq("#link-post_tag").click(function(){
		if(get_tag_ajax == 0){
			jq.post("<?php echo PHP_SELF; ?>?m=tag.ajaxGetTag",
				function(data){	
	...
	jq('#tag_add_button').click(function(){
		...
		if(new_tag == '添加新Tag'){
			...
			jq.post("<?php echo PHP_SELF; ?>?m=tag.ajaxSaveTag", {tag_name:new_tag},
				function(data){
					jq('#new_tag').val('');
					var obj=eval("("+data+")");//转换为json对象 			
					jq.each(obj,function(j, n){
...
<form name="list_form" method="POST" action="<?php echo PHP_SELF; ?>?m=article.article_do&page=<?php echo $pageCurrent;?>" onSubmit="return checkDelForm();">
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="t_list">
      <tbody><tr>
      <th>&nbsp;</th>
        <th width="5%">编号</th>
      ...
      <td><input type="checkbox" value="<?php echo $v->article_id;?>" name="id_a[]"></td>
      <td><?php echo $v->article_id;?></td>
      <td><a href="<?php echo PHP_SELF; ?>?m=archives.arclist&channelid=<?php echo $v->channel;?>&cat_id=<?php echo $v->cat_id;?>"><?php echo $v->cat_name;?></a></td>
      <td><a href="<?php echo PHP_SELF; ?>?m=archives.arclist&channelid=<?php echo $v->channel;?>"><?php echo $v->channeltype;?></a></td>      
      <td class="td_left"><?php echo $v->title;?></td> 
      <td><?php echo $v->status;?></td> 
      <td><?php echo $v->time;?></td> 
      <td><a href="<?php echo PHP_SELF; ?>?m=archives.edit&channelid=<?php echo $v->channel;?>&aid=<?php echo $v->article_id;?>">修改</a> | <a href="<?php echo PHP_SELF; ?>?m=article.article_do&action=del&aid=<?php echo $v->article_id;?>" onclick="{if(confirm('删除将包括该信息，确定删除吗?')){return true;}return false;}">删除</a></td>
      </tr>
      <?php } ?>      
