...
class Easemob
{
    ...
    public function setEasemob_token_file( $easemob_token_file )
    {
        $this->easemob_token_file = $easemob_token_file;
    }
    ...
    public function getToken()
    {
        ...
        $url = $this->url . "token";
        $fp = fopen( $this->easemob_token_file, 'r+' );
        if ( $fp ) {
        ...
        $result = $this->postCurl( $url, $option, $head = 0 );
        $result [ 'expires_in' ] = $result [ 'expires_in' ] + time();
        $fp = fopen( $this->easemob_token_file, 'w' );
        fwrite( $fp, serialize( $result ) );
        fclose( $fp );
