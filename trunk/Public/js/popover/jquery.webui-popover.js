...
;(function ( $, window, document, undefined ) {
		...
		var	defaults = {
					...
					template:'<div class="webui-popover">'+
								...
								'</div>'+
							'</div>',
					afterLoad: null,//增加afterLoad的回调 add by wentao 
					onClose: null,//增加onClose的回调 add by wentao 
					bodyClickHide: true //增加body click hide配置。在其他地方点击是否关闭 add by wentao 
		};
		...
		function WebuiPopover ( element, options ) {
				...
				this._show_status = false;//add by wentao
				this.init();
		}
		...
		WebuiPopover.prototype = {
				...
				//add afterLoad callback.add by wentao start
				afterLoad:function(){
					var that = this,
						afterLoadCallback = that.options.afterLoad,
						$target = this.getTarget();
						
					if ($.isFunction(afterLoadCallback)) {
						afterLoadCallback.call(this.$element,$target);
					}					
				},
				//add afterLoad callback.add by wentao end
				//add onClose callback.add by wentao start
				onClose:function(){
					var that = this,
						onCloseCallback = that.options.onClose,
						$target = this.getTarget();
						
					if ($.isFunction(onCloseCallback)) {
						onCloseCallback.call(this.$element,$target);
					}					
				}
				//add onClose callback.add by wentao end
		};
