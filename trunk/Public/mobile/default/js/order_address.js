$(function() {
    ...
    if ($("#tele").val() == '') {
        $("#tele").val(global_member_info.mobile);
    }
    region.init(); //初始化Region三级联动

    $("#js_okBtn").click(function() { //提交按钮
        var nam = $("#nam").val();
        if (!nam) {
            alert('请填写收货人姓名!');
            $("#nam").focus();
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success == true) {
                    ...
                    var item_uid = global_member_info.uid;
                    location.href = $.cookie('back_order_address_url') + '&address_id=' + address_id;
                } else {
                    alert(data.message);
                }
            }
