 ...
 * 
 */
class accountAction extends service_Controller_manage
{
    ...
    public function _init()
    {
        ...
        $this->sms_type_array = array( 1, 2, 3 );
    }
    ...
    {
        $model = new service_account_Login_manage();
        ...
        $referer_url = (strpos( $referer_url, "register" ) === false) ? $referer_url : INDEX_URL;
        $referer_url = (strpos( $referer_url, "password" ) === false) ? $referer_url : INDEX_URL;
        $referer_url = (strpos( $referer_url, "forget" ) === false) ? $referer_url : INDEX_URL;

        setcookie( 'Referer', $referer_url, 3600 + time(), '/', $GLOBALS [ 'TmacConfig' ] [ 'Cookie' ] [ 'domain' ] );
        $this->assign( 'referer_url', $referer_url ); //跳转链接
    ...
    public function loginout()
    {
        $model = new service_account_Login_manage();
        $model->loginOut();
        $redirect = MOBILE_URL .PHP_SELF. '?m=account.login';
    ...
    public function login_do()
    {
        ...
        if ( $login_data->member_type == service_Member_base::member_type_buyer ) {
            throw new ApiException( '分销商/供应商的用户身份才能登录系统，请联系银品惠客服' );
        }
        if ( $order_cart == 1 ) {
            $order_model = new service_order_Cart_manage();
    ...
    public function send_verify_code()
    {
        ...
        //检查用户是否存在
        if ( $sms_type == service_account_Register_manage::sms_type_register && $model->checkMobileRepeat() ) {
    ...
    public function password()
    {
        ...
        $model->setMobile( $mobile );
        $model->setSms_captcha( $sms_captcha );
        $model->setSms_type( service_account_Register_manage::sms_type_forget_password );
        $model->setPassword( $password );
