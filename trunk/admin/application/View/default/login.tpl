...
<style>
body { background:url({$BASE_V}images/login_bg.gif) 0 0 repeat-x;}
</style>
...
function reloadcode()
{
	$('img11').setAttribute('src','/{PHP_SELF}?m=tool.Captcha&'+Math.random())
}
</script>
</head>
...
<div id="login">
  ...
  <form id="myform" name="myform" method="post" action="{PHP_SELF}?m=login.check" onSubmit="return CheckPost();">
    用户名
    ...
    　验证码
    <input name="yzm" type="text" style="width:50px" id="yzm" /><img src="{PHP_SELF}?m=tool.Captcha" id="img11" class="yzm" onclick="reloadcode();">
