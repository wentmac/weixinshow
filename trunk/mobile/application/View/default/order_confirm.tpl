...
<body>
  ...
  <section id="order_wrap">
    ...
    <section id="order_seller">
      <a id="o_seller_a" class="block rel">
        <!--{if $cart_list}-->
        <p id="o_seller" data-id='{$cart_list[0]->item_uid}' class="over_hidden ellipsis">{$cart_list[0]->shop_name}</p>
        <!--{/if}-->
      </a>
    </section>
    ...
    <!--{loop $cart_list $v}-->
        <li class="o_li li_for_pay" data-cart-id='{$v->cart_id}' data-price="{$v->item_price}" data-item-id="{$v->item_id}" data-sku-id="{$v->goods_sku_id}" data-count="{$v->item_number}" data-limit_discount_id="">
          <a href="{MOBILE_URL}goods/{$v->goods_id}.html" class="o_a rel block"><img src="{$v->goods_image_id}" width="50" height="50" class="left">
            <p class="o_name">{$v->goods_name}<!--{if $v->is_integral==1}--><font color="red">[积分抵扣]</font><!--{/if}--></p>
            <p class="o_a_sku over_hidden ellipsis">型号:&nbsp;{$v->goods_sku_name}</p>
            <p class="o_a_pri i_pri abs r_txt">¥{$v->item_price}</p>
            <p class="o_a_count abs r_txt">x&nbsp;{$v->item_number}</p>
          </a>
        </li>
    <!--{/loop}-->
      </ul>
    ...
    <section id="hongbao" class="hongbao_current hide">
      <div id="hongbao_content">
        <p class="left">{$config[cfg_webname]}购物券</p>
    ...
    <section id="order_money" class="hide" style="display: block;">
      ...
      <p id="delivery_service_money" class="r_txt">共${echo count($cart_list);}件商品，商品总额:&nbsp;<span id="delivery_service_money_span" class="i_pri">¥{$total_price}</span>
      	
      </p>
      <p id="shipping_fee" class="r_txt">邮费: &nbsp;<span class="i_pri">+¥{$shipping_fee}</span></p>      
      <p id="count_money_wrap" class="r_txt">应付总额:&nbsp;<span class="i_pri">¥{$order_payable_amount}</span></p>
      <!--{if $use_integral==true}-->
      <p id="use_integral" class="r_txt">积分抵扣: &nbsp;<span class="i_pri">-¥{$use_available_integral}</span></p>
      <!--{/if}-->      
      <p id="discount_money" class="r_txt hide"></p>
      <p id="free_postage" class="r_txt hide"></p>
    </section>
    <section id="order_remark">
      <p class="address_p rel <!--{if $agent_lock==1}-->hide<!--{/if}-->">
    ...
    <footer id="order_footer" class="wrap fix hide" style="display: block;">
      <div id="order_btns" class="wrap rel margin_auto">
        <div id="order_btns_inner">
          ...
          <p id="last_money" class="r_txt"><!--{if $use_integral==true}-->我的可用积分：<span class="i_pri">{$available_integral}</span>   &nbsp;&nbsp;<!--{/if}-->应付总额:&nbsp;<span class="i_pri" id="last_money_show">¥{$total_amount}</span></p>
        </div>
      </div>
    </footer>
  </section>
  ...
  var global_item_uid = '{$item_uid}';
  var global_address_id = '{$address_info->address_id}';
  var global_goods_uid = '{$goods_uid}';
  </script>
  <script type="text/javascript" src="{$BASE_V}js/order_confirm.js?v=1"></script>
</body>
