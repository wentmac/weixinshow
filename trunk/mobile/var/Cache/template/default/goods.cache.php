...
0
|| self::check('default\goods.tpl', 'D:\Web\Witkey\wwwroot\yph\trunk\mobile\application\View\default\goods.tpl', 1465815661)
|| self::check('default\goods.tpl', 'D:\Web\Witkey\wwwroot\yph\trunk\mobile\application\View\default\inc/header.tpl', 1465815661)
|| self::check('default\goods.tpl', 'D:\Web\Witkey\wwwroot\yph\trunk\mobile\application\View\default\inc/js.tpl', 1465815661)
;?>
<!DOCTYPE html>
<html>
	...
	<head>
		...
		<link href="<?php echo STATIC_URL; ?>common/assets/css/amazeui.css" type="text/css" rel="stylesheet">
		...
		<link href="<?php echo $BASE_V;?>css/item/item.css" type="text/css" rel="stylesheet">
		<link href="<?php echo $BASE_V;?>css/common/itemListTemplate.css" type="text/css" rel="stylesheet">
		<link href="<?php echo $BASE_V;?>css/market/recommend.css" type="text/css" rel="stylesheet">
		<style>
...
<style>img{border:0;}ul,li{padding:0;margin:0;}.box_lanrenzhijia {z-index:99;right:0;width:128px;height:195px;position:absolute;}.box_lanrenzhijia .press{right:0;width:36px;cursor:pointer;position:absolute;height:128px;}.box_lanrenzhijia .zzjs_net_list{left:0;width:131px;position:absolute;height:195px;background:url(images/20120321lanrenzhijia_1.gif) no-repeat left center;}.box_lanrenzhijia .zzjs_net_list ul{padding:37px 0 0 21px;}.box_lanrenzhijia .zzjs_net_list li{height:26px;margin-bottom:3px;_margin-bottom:3px; list-style-type:none;}</style>		
	...
	<body style="padding-bottom: 60px;">
		<div id="item_show_wrap" style="height: auto; overflow: visible;"><header id="common_hd" class="c_txt rel">
	...
	<div class="my_shop for_gaq" data-for-gaq="点击我的<?php echo $config['cfg_webname'];?>;详情页"><span></span></div>
</header><div id="item_wrap_loading" class="loading" style="display: none;">&nbsp;</div>
			<div id="item_info_for_show_wrap" class="hide" style="display: block;">
				
				<section id="item_info" class="rel">
					<div class="slider">
							<ul>
								<?php if(is_array($goods_image_array)) foreach($goods_image_array AS $item) { ?>
								<li>
									<img src="<?php echo $item->goods_image_id;?>" width="100%">
								</li>
								<?php } ?>
							</ul>
					</div>
					...
					<article id="item_show" class="loading c_txt rel">
						<!--<img width="100%" src="<?php echo $goods_info->goods_image_id;?>" data-share-src="<?php echo $goods_info->goods_image_id;?>">-->
						<div id="hd_bg_item" class="abs hide" style="display: block;"></div><em id="sold" class="abs">收藏&nbsp;<span id="collect-number"><?php echo $goods_info->collect_count;?></span></em>
						<div id="favorite" class="favorite for_gaq hide" data-for-gaq="收藏商品">收藏</div>
					</article>
					<h2 id="item_name"><strong><?php echo $goods_info->goods_name;?></strong><br>
        
        </h2>
					<div id="item_price_wrap" class="rel">
						<p>							
							<span id="item_price" class="i_pri"><?php echo $goods_info->goods_price;?></span> 
							<?php if($goods_info->goods_price<>$goods_info->price_source) { ?>
							<span id="seckill_price"><del>￥<?php echo $goods_info->price_source;?></del></span> 
							<?php } ?>
							<?php if($goods_info->is_integral==1) { ?>
						...
						<!-- <?php if(empty($goods_info->shipping_fee)) { ?>-->
						<p id="free_delivery" class="hide rel" style="display: block;">包邮 <em id="free_delivery_em">偏远地区请联系<?php echo $config['cfg_webname'];?>主</em></p>
						<!-- <?php } else { ?> -->
						<p id="free_delivery" class="hide rel" style="display: block;">邮费:<em id="free_delivery_em">￥<?php echo $goods_info->shipping_fee;?></em></p>
						<?php } ?>
				...
				<section id="item_detail">
					<div id="detail_wrap">
						...
						<?php if(is_array($goods_image_array)) foreach($goods_image_array AS $k => $v) { ?>
						<img src="<?php echo $v->goods_image_id;?>" width="100%">
						<?php } ?>						
						<?php echo $goods_info->goods_desc;?>
					</div>
					<a href="<?php echo MOBILE_URL; ?>" target="_blank" id="iWantAShop" class="block c_txt for_gaq rel" data-for-gaq="详情页－我也要开<?php echo $config['cfg_webname'];?>" style="margin-top: 40px; display:none;">
					</a>
				</section>
			</div>
				...
				<div class="am-modal-bd am-text-left">
					<div class="select_sku_pro_info">
						<div class="sku_img am-fl">
							<img src="<?php echo $goods_info->goods_image_id;?>" width="50" height="50">
						</div>
						<div id="sku_pro_tit" class="sku_pro_tit"><?php echo $goods_info->goods_name;?></div>
					...
					无含邮费
					<!-- <?php } else { ?> -->
					<div id="shipping_fee" class="left sku_item_tit am-padding-left-sm">邮费:</div>￥<?php echo $goods_info->shipping_fee;?>
					<?php } ?>
					<hr>
		...
		<div class="box_lanrenzhijia" id="qqbox_zzjs" style="top: 379px;">
			...
			<div id="menu_zzjs_net" style="display: block;">
				<img src="<?php echo $BASE_V;?>image/qq.gif" class="press" alt="在线客服" src="http://wpa.qq.com/pa?p=2:787872805:41" onclick="javascript:window.open('http://wpa.qq.com/msgrd?v=3&uin=787872805&site=qq&menu=yes', '_blank', 'height=502, width=644,toolbar=no,scrollbars=no,menubar=no,status=no');" border="0">
			</div>
		</div>
...
<script type="text/javascript">
	...
	var gloabl_item_id = <?php echo $goods_info->item_id;?>;
	var gloabl_item_price = '<?php echo $goods_info->goods_price;?>';
	var gloabl_item_stock = <?php echo $goods_info->goods_stock;?>;
	var global_goods_name = '<?php echo $goods_info->goods_name;?>';
	var global_goods_spec_array = <?php echo $goods_spec_array;?>;
	var global_goods_sku_array = <?php echo $goods_sku_array;?>;
	var global_sku_init = 0;
	var global_sku_current = "";
	var global_sku_current_obj = new Object();
	var global_goods_type = <?php echo $goods_info->goods_type;?>;
...
<script src="<?php echo $BASE_V;?>js/common.js?v=201635" type="text/javascript"></script><script src="<?php echo $BASE_V;?>js/mobile_slider.js" type="text/javascript"></script>
<script src="<?php echo STATIC_URL; ?>common/assets/js/amazeui.min.js" type="text/javascript"></script>
<script src="<?php echo STATIC_URL; ?>js/modal_html.js" type="text/javascript"></script>
<script src="<?php echo $BASE_V;?>js/jquery.lazyload.js" type="text/javascript"></script>
<script>
...
<script src="<?php echo $BASE_V;?>js/item.js?v=20160101" type="text/javascript"></script>
